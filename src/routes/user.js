...
router.get('/panel', isAuthenticated, async (req, res) => {
  ...
  const recentFavorites = await db.prepare(`
    SELECT l.*, b.name as brand_name, m.name as model_name,
           (SELECT url FROM listing_images WHERE listing_id = l.id AND is_primary = 1 LIMIT 1) as image
    FROM favorites f
  ...
  const forumActivity = await db.prepare(`
...
router.get('/ilanlarim', isAuthenticated, async (req, res) => {
  const db = getDb();
  const userId = req.session.user.id;
  ...
  const statusCounts = {
    all: (await db.prepare('SELECT COUNT(*) as c FROM listings WHERE user_id=?').get(userId)).c,
    active: (await db.prepare("SELECT COUNT(*) as c FROM listings WHERE user_id=? AND status='active'").get(userId)).c,
    pending: (await db.prepare("SELECT COUNT(*) as c FROM listings WHERE user_id=? AND status='pending'").get(userId)).c,
    sold: (await db.prepare("SELECT COUNT(*) as c FROM listings WHERE user_id=? AND status='sold'").get(userId)).c,
    draft: (await db.prepare("SELECT COUNT(*) as c FROM listings WHERE user_id=? AND status='draft'").get(userId)).c,
  };
  ...
  res.render('pages/kullanici-paneli', {
    ...
    userProfile: await db.prepare('SELECT * FROM users WHERE id = ?').get(userId),
    userPage: 'ilanlarim', userData: { listings, statusCounts, filters: req.query }
  });
});

// Favorilerim - GET /kullanici/favorilerim
router.get('/favorilerim', isAuthenticated, async (req, res) => {
  const db = getDb();
  const userId = req.session.user.id;
...
router.get('/mesajlar', isAuthenticated, async (req, res) => {
  const db = getDb();
  const userId = req.session.user.id;
...
router.get('/mesajlar/:userId', isAuthenticated, async (req, res) => {
  const db = getDb();
  ...
  res.render('pages/kullanici-paneli', {
    ...
    userProfile: await db.prepare('SELECT * FROM users WHERE id = ?').get(myId),
    userPage: 'mesaj-detay', userData: { messages, otherUser }
  });
});

// Mesaj gönder (form POST) - POST /kullanici/mesajlar/:userId
router.post('/mesajlar/:userId', isAuthenticated, async (req, res) => {
  const db = getDb();
  ...
  await db.prepare("INSERT INTO notifications (user_id, type, title, message, link) VALUES (?, 'message', 'Yeni Mesaj', ?, '/kullanici/mesajlar')")
...
router.get('/bildirimler', isAuthenticated, async (req, res) => {
  const db = getDb();
  const userId = req.session.user.id;
  ...
  const unreadCount = (await db.prepare('SELECT COUNT(*) as c FROM notifications WHERE user_id = ? AND is_read = 0').get(userId)).c;
...
router.get('/randevular', isAuthenticated, async (req, res) => {
  const db = getDb();
  const userId = req.session.user.id;
...
router.post('/profil', isAuthenticated, async (req, res) => {
  const db = getDb();
...
router.post('/sifre', isAuthenticated, async (req, res) => {
  const db = getDb();
